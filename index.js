...
async function handleOrderText(replyToken, text) {
  try {
    ...
    const response = await anthropic.messages.create({
      model: 'claude-haiku-4-5-20251001',
      max_tokens: 1500,
      messages: [{ role: 'user', content: 'อ่านข้อความออเดอร์นี้แล้วตอบเป็น JSON เท่านั้น ห้ามมี markdown {"order_number":"","order_numbers":[],"customer_name":"","platform":"","order_date":"","note":"","items":[{"curtain_type":"","color_code":"","color_name":"","eye_color":"","rail_floors":"","rail_head":"","width":0,"height":0,"quantity":0,"unit":"ผืน"}]} order_numbers=array ของเลขออเดอร์ทั้งหมดในข้อความ เช่น ["260427VHSUSA6K","2604293GTKSM35"], order_number=เลขแรกใน order_numbers, customer_name=ชื่อลูกค้าหลัง platform, platform=Tiktok/Shopee/Facebook/LineOA/Lazada, order_date=วันที่ถ้าปีไม่ชัดใช้ 2026, items แยกทุกรายการ curtain_type=ประเภท rail_floors=จำนวนชั้นถ้าเป็นราง rail_head=หัวรางถ้ามี width/height อ่านเป็นเมตรให้ครบทุกหลักรวมทศนิยม เช่น ก1.39.25 หมายถึง 1.3925 เมตร ส2.69.5 หมายถึง 2.695 เมตร ถ้าเป็นรางใส่แค่ width height=0\n\nข้อความ:\n' + text }]
    });
